...
public interface OrderItemRepository extends JpaRepository<OrderItemEntity, Long> {
    @Query(value = "SELECT oi.id as id, oi.count as quantity, si.title as itemName, si.price as price, si.thumbnail as imageUrl FROM order_item oi LEFT JOIN shopping_item si on si.id = oi.item_id where oi.order_id in (:orderIds)", nativeQuery = true)
